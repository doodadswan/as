...
$query_getE = "SELECT * FROM artists WHERE artists.LastName LIKE 'E%' ORDER BY artists.LastName";
$getE = mysql_query($query_getE, $artists) or die(mysql_error());
$row_getE = mysql_fetch_assoc($getE);
$totalRows_getE = mysql_num_rows($getE);
?>
...
<title>Visual Artist Database | E</title>
...
<meta name="description" content="Visual Artist Database" />
<meta name="author" content="Alex Taylor Swan" />
<title>AlexSwan.com | Visual Artist Database | E</title>
<style type="text/css" media="screen">
@import url("css/style.css");
</style>
<meta name="google-translate-customization" content="f91f6b59f687e07-728df0675eb22298-gf4646ea128908eee-c"></meta>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getE['Name']; ?></td>
      	    <td><?php echo $row_getE['Movement']; ?></td>
      	    <td>
				<?php if ($row_getE['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getE['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getE['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getE['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getE['CityBirth']; ?></td>
      	    <td><?php echo $row_getE['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getE = mysql_fetch_assoc($getE)); ?>
    </tbody>
    </table>
  </section>
