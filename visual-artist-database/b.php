...
$query_getB = "SELECT * FROM artists WHERE artists.LastName LIKE 'B%' ORDER BY artists.LastName";
$getB = mysql_query($query_getB, $artists) or die(mysql_error());
$row_getB = mysql_fetch_assoc($getB);
$totalRows_getB = mysql_num_rows($getB);
?>
...
<title>Visual Artist Database | B</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getB['Name']; ?></td>
      	    <td><?php echo $row_getB['Movement']; ?></td>
      	    <td>
				<?php if ($row_getB['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getB['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getB['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getB['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getB['CityBirth']; ?></td>
      	    <td><?php echo $row_getB['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getB = mysql_fetch_assoc($getB)); ?>
    </tbody>
    </table>
  </section>
<footer><p>&copy; 2003-<?php echo(date("Y")) ?> <a href="http://www.alexswan.com" target="_blank">Alex Swan</a> and GeminiGraphix</p></footer>
