...
$query_getC = "SELECT * FROM artists WHERE artists.LastName LIKE 'C%' ORDER BY artists.LastName";
$getC = mysql_query($query_getC, $artists) or die(mysql_error());
$row_getC = mysql_fetch_assoc($getC);
$totalRows_getC = mysql_num_rows($getCS);
?>
...
<title>Visual Artist Database | C</title>
...
<meta name="description" content="Visual Artist Database" />
<meta name="author" content="Alex Taylor Swan" />
<title>AlexSwan.com | Visual Artist Database | C</title>
<style type="text/css" media="screen">
@import url("css/style.css");
</style>
<meta name="google-translate-customization" content="f91f6b59f687e07-728df0675eb22298-gf4646ea128908eee-c"></meta>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getC['Name']; ?></td>
      	    <td><?php echo $row_getC['Movement']; ?></td>
      	    <td>
				<?php if ($row_getC['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getC['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getC['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getC['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getC['CityBirth']; ?></td>
      	    <td><?php echo $row_getC['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getC = mysql_fetch_assoc($getC)); ?>
    </tbody>
    </table>
  </section>
