...
$query_getR = "SELECT * FROM artists WHERE artists.LastName LIKE 'R%' ORDER BY artists.LastName";
$getR = mysql_query($query_getR, $artists) or die(mysql_error());
$row_getR = mysql_fetch_assoc($getR);
$totalRows_getR = mysql_num_rows($getR);
?>
...
<title>Visual Artist Database | R</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getR['Name']; ?></td>
      	    <td><?php echo $row_getR['Movement']; ?></td>
      	    <td>
				<?php if ($row_getR['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getR['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getR['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getR['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getR['CityBirth']; ?></td>
      	    <td><?php echo $row_getR['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getR = mysql_fetch_assoc($getR)); ?>
    </tbody>
    </table>
  </section>
