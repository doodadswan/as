...
$query_getK = "SELECT * FROM artists WHERE artists.LastName LIKE 'K%' ORDER BY artists.LastName";
$getK = mysql_query($query_getK, $artists) or die(mysql_error());
$row_getK = mysql_fetch_assoc($getK);
$totalRows_getK = mysql_num_rows($getK);
?>
...
<title>Visual Artist Database | K</title>
...
<li><a href="f.php">F</a></li>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getK['Name']; ?></td>
      	    <td><?php echo $row_getK['Movement']; ?></td>
      	    <td>
				<?php if ($row_getK['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getK['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getK['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getK['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getK['CityBirth']; ?></td>
      	    <td><?php echo $row_getK['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getK = mysql_fetch_assoc($getK)); ?>
    </table>
  </section>
