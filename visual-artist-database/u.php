...
$query_getU = "SELECT * FROM artists WHERE artists.LastName LIKE 'U%' ORDER BY artists.LastName";
$getU = mysql_query($query_getU, $artists) or die(mysql_error());
$row_getU = mysql_fetch_assoc($getU);
$totalRows_getU = mysql_num_rows($getU);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Visual Artist Database | U</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getU['Name']; ?></td>
      	    <td><?php echo $row_getU['Movement']; ?></td>
      	    <td>
				<?php if ($row_getU['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getU['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getU['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getU['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getU['CityBirth']; ?></td>
      	    <td><?php echo $row_getU['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getU = mysql_fetch_assoc($getU)); ?>
    </tbody>
    </table>
  </section>
