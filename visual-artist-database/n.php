...
$query_getN = "SELECT * FROM artists WHERE artists.LastName LIKE 'N%' ORDER BY artists.LastName";
$getN = mysql_query($query_getN, $artists) or die(mysql_error());
$row_getN = mysql_fetch_assoc($getN);
$totalRows_getN = mysql_num_rows($getN);
?>
...
<title>Visual Artist Database | N</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getN['Name']; ?></td>
      	    <td><?php echo $row_getN['Movement']; ?></td>
      	    <td>
				<?php if ($row_getN['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getN['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getN['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getN['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getN['CityBirth']; ?></td>
      	    <td><?php echo $row_getN['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getN = mysql_fetch_assoc($getN)); ?>
    </tbody>
    </table>
  </section>
