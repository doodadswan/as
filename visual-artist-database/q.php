...
$query_getQ = "SELECT * FROM artists WHERE artists.LastName LIKE 'Q%' ORDER BY artists.LastName";
$getQ = mysql_query($query_getQ, $artists) or die(mysql_error());
$row_getQ = mysql_fetch_assoc($getQ);
$totalRows_getQ = mysql_num_rows($getQ);
?>
...
<title>Visual Artist Database | Q</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getQ['Name']; ?></td>
      	    <td><?php echo $row_getQ['Movement']; ?></td>
      	    <td>
				<?php if ($row_getQ['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getQ['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getQ['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getQ['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getQ['CityBirth']; ?></td>
      	    <td><?php echo $row_getQ['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getQ = mysql_fetch_assoc($getQ)); ?>
    </tbody>
    </table>
  </section>
