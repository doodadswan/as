...
$query_getL = "SELECT * FROM artists WHERE artists.LastName LIKE 'L%' ORDER BY artists.LastName";
$getL = mysql_query($query_getL, $artists) or die(mysql_error());
$row_getL = mysql_fetch_assoc($getL);
$totalRows_getL = mysql_num_rows($getL);
?>
...
<title>Visual Artist Database | L</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getL['Name']; ?></td>
      	    <td><?php echo $row_getL['Movement']; ?></td>
      	    <td>
				<?php if ($row_getL['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getL['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getL['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getL['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getL['CityBirth']; ?></td>
      	    <td><?php echo $row_getL['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getL = mysql_fetch_assoc($getL)); ?>
    </tbody>
    </table>
  </section>
