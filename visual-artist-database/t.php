...
$query_getT = "SELECT * FROM artists WHERE artists.LastName LIKE 'T%' ORDER BY artists.LastName";
$getT = mysql_query($query_getT, $artists) or die(mysql_error());
$row_getT = mysql_fetch_assoc($getT);
$totalRows_getT = mysql_num_rows($getT);
?>
...
<title>Visual Artist Database | T</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getT['Name']; ?></td>
      	    <td><?php echo $row_getT['Movement']; ?></td>
      	    <td>
				<?php if ($row_getT['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getT['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getT['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getT['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getT['CityBirth']; ?></td>
      	    <td><?php echo $row_getT['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getT = mysql_fetch_assoc($getT)); ?>
    </tbody>
    </table>
  </section>
