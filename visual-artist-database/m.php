...
$query_getM = "SELECT * FROM artists WHERE artists.LastName LIKE 'M%' ORDER BY artists.LastName";
$getM = mysql_query($query_getM, $artists) or die(mysql_error());
$row_getM = mysql_fetch_assoc($getM);
$totalRows_getM = mysql_num_rows($getM);
?>
...
<title>Visual Artist Database | M</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getM['Name']; ?></td>
      	    <td><?php echo $row_getM['Movement']; ?></td>
      	    <td>
				<?php if ($row_getM['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getM['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getM['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getM['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getM['CityBirth']; ?></td>
      	    <td><?php echo $row_getM['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getM = mysql_fetch_assoc($getM)); ?>
    </table>
  </section>
