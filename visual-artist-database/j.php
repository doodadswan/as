...
$query_getJ = "SELECT * FROM artists WHERE artists.LastName LIKE 'J%' ORDER BY artists.LastName";
$getJ = mysql_query($query_getJ, $artists) or die(mysql_error());
$row_getJ = mysql_fetch_assoc($getJ);
$totalRows_getJ = mysql_num_rows($getJ);
?>
...
<title>Visual Artist Database | J</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getJ['Name']; ?></td>
      	    <td><?php echo $row_getJ['Movement']; ?></td>
      	    <td>
			  <?php if ($row_getJ['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getJ['YearBirth']; } ?>
            </td>
      	    <td>
			  <?php if ($row_getJ['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getJ['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getJ['CityBirth']; ?></td>
      	    <td><?php echo $row_getJ['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getJ = mysql_fetch_assoc($getJ)); ?>
    </tbody>
    </table>
  </section>
