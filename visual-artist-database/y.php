...
$query_getP= "SELECT * FROM artists WHERE artists.LastName LIKE 'P%' ORDER BY artists.LastName";
$getP = mysql_query($query_getP, $artists) or die(mysql_error());
$row_getP = mysql_fetch_assoc($getP);
$totalRows_getP = mysql_num_rows($getP);
?>
...
<title>Visual Artist Database | Y</title>
...
<!--Google Adsense Ends-->
</head><?php require_once('../Connections/artists.php'); ?>
<?php
if (!function_exists("GetSQLValueString")) {
function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
...
mysql_select_db($database_artists, $artists);
$query_getY = "SELECT * FROM artists WHERE artists.LastName LIKE 'Y%' ORDER BY artists.LastName";
$getY = mysql_query($query_getY, $artists) or die(mysql_error());
$row_getY = mysql_fetch_assoc($getY);
$totalRows_getY = mysql_num_rows($getY);
?>
<!DOCTYPE html>
<html lang="en-us">
<head>
...
<meta name="description" content="Visual Artist Database" />
<meta name="author" content="Alex Taylor Swan" />
<title>AlexSwan.com | Visual Artist Database | Y</title>
<style type="text/css" media="screen">
@import url("css/style.css");
</style>
<meta name="google-translate-customization" content="f91f6b59f687e07-728df0675eb22298-gf4646ea128908eee-c"></meta>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getY['Name']; ?></td>
      	    <td><?php echo $row_getY['Movement']; ?></td>
      	    <td>
				<?php if ($row_getY['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getY['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getY['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getY['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getY['CityBirth']; ?></td>
      	    <td><?php echo $row_getY['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getY = mysql_fetch_assoc($getY)); ?>
    </tbody>
    </table>
  </section>
