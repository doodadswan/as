...
$query_getW = "SELECT * FROM artists WHERE artists.LastName LIKE 'W%' ORDER BY artists.LastName";
$getW = mysql_query($query_getW, $artists) or die(mysql_error());
...
<title>AlexSwan.com | Visual Artist Database | W</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getW['Name']; ?></td>
      	    <td><?php echo $row_getW['Movement']; ?></td>
      	    <td>
				<?php if ($row_getW['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getW['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getW['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getW['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getW['CityBirth']; ?></td>
      	    <td><?php echo $row_getW['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getW = mysql_fetch_assoc($getW)); ?>
    </tbody>
    </table>
  </section>
