...
$query_getZ= "SELECT * FROM artists WHERE artists.LastName LIKE 'Z%' ORDER BY artists.LastName";
$getZ = mysql_query($query_getZ, $artists) or die(mysql_error());
$row_getZ = mysql_fetch_assoc($getZ);
$totalRows_getZ = mysql_num_rows($getZ);
?>
...
<title>Visual Artist Database | Z</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getZ['Name']; ?></td>
      	    <td><?php echo $row_getZ['Movement']; ?></td>
      	    <td>
				<?php if ($row_getZ['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getZ['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getZ['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getZ['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getZ['CityBirth']; ?></td>
      	    <td><?php echo $row_getZ['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getZ = mysql_fetch_assoc($getZ)); ?>
    </tbody>
    </table>
  </section>
