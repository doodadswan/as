...
$query_getH = "SELECT * FROM artists WHERE artists.LastName LIKE 'H%' ORDER BY artists.LastName";
$getH = mysql_query($query_getH, $artists) or die(mysql_error());
$row_getH = mysql_fetch_assoc($getH);
$totalRows_getH = mysql_num_rows($getH);
?>
...
<title>Visual Artist Database | H</title>
...
<meta name="description" content="Visual Artist Database" />
<meta name="author" content="Alex Taylor Swan" />
<title>AlexSwan.com | Visual Artist Database | H</title>
<style type="text/css" media="screen">
@import url("css/style.css");
</style>
<meta name="google-translate-customization" content="f91f6b59f687e07-728df0675eb22298-gf4646ea128908eee-c"></meta>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getH['Name']; ?></td>
      	    <td><?php echo $row_getH['Movement']; ?></td>
      	    <td>
			  <?php if ($row_getH['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getH['YearBirth']; } ?>
            </td>
      	    <td>
			  <?php if ($row_getH['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getH['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getH['CityBirth']; ?></td>
      	    <td><?php echo $row_getH['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getH = mysql_fetch_assoc($getH)); ?>
    </tbody>
    </table>
  </section>
