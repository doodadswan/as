...
$query_getG = "SELECT * FROM artists WHERE artists.LastName LIKE 'G%' ORDER BY artists.LastName";
$getG = mysql_query($query_getG, $artists) or die(mysql_error());
$row_getG = mysql_fetch_assoc($getG);
$totalRows_getG = mysql_num_rows($getG);
?>
...
<title>Visual Artist Database | G</title>
...
<meta name="description" content="Visual Artist Database" />
<meta name="author" content="Alex Taylor Swan" />
<title>AlexSwan.com | Visual Artist Database | G</title>
<style type="text/css" media="screen">
@import url("css/style.css");
</style>
<meta name="google-translate-customization" content="f91f6b59f687e07-728df0675eb22298-gf4646ea128908eee-c"></meta>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getG['Name']; ?></td>
      	    <td><?php echo $row_getG['Movement']; ?></td>
      	    <td>
			  <?php if ($row_getG['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getG['YearBirth']; } ?>
            </td>
      	    <td>
			  <?php if ($row_getG['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getG['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getG['CityBirth']; ?></td>
      	    <td><?php echo $row_getG['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getG = mysql_fetch_assoc($getG)); ?>
    </tbody>
    </table>
  </section>
