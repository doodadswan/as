...
$query_getA = "SELECT * FROM artists WHERE artists.LastName LIKE 'A%' ORDER BY artists.LastName";
$getA = mysql_query($query_getA, $artists) or die(mysql_error());
$row_getA = mysql_fetch_assoc($getA);
$totalRows_getA = mysql_num_rows($getA);
?>
<!DOCTYPE html>
<html lang="en-us">
<head>
...
<title>AlexSwan.com | Visual Artist Database | A</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getA['Name']; ?></td>
      	    <td><?php echo $row_getA['Movement']; ?></td>
      	    <td>
				<?php if ($row_getA['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getA['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getA['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getA['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getA['CityBirth']; ?></td>
      	    <td><?php echo $row_getA['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getA = mysql_fetch_assoc($getA)); ?>
    </table>
  </section>
<footer><p>&copy; 2003-<?php echo(date("Y")) ?> alex swan and <a href="http://www.geminigraphix.com" target="_blank">geminigraphix</a></p></footer>
