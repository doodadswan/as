...
$query_getV = "SELECT * FROM artists WHERE artists.LastName LIKE 'V%' ORDER BY artists.LastName";
$getV = mysql_query($query_getV, $artists) or die(mysql_error());
$row_getV = mysql_fetch_assoc($getV);
$totalRows_getV = mysql_num_rows($getV);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>AlexSwan.com | Visual Artist Database | V</title>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><a href="details.php?Name=<?php echo $row_getV['Name']; ?>"><?php echo $row_getV['Name']; ?></a></td>
      	    <td><?php echo $row_getV['Movement']; ?></td>
      	    <td>
				<?php if ($row_getV['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getV['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getV['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getV['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getV['CityBirth']; ?></td>
      	    <td><?php echo $row_getV['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getV = mysql_fetch_assoc($getV)); ?>
    </tbody>
    </table>
  </section>
