...
$query_getF = "SELECT * FROM artists WHERE artists.LastName LIKE 'F%' ORDER BY artists.LastName";
$getF = mysql_query($query_getF, $artists) or die(mysql_error());
$row_getF = mysql_fetch_assoc($getF);
$totalRows_getF = mysql_num_rows($getF);
?>
...
<title>Visual Artist Database | F</title>
...
<meta name="description" content="Visual Artist Database" />
<meta name="author" content="Alex Taylor Swan" />
<title>AlexSwan.com | Visual Artist Database | F</title>
<style type="text/css" media="screen">
@import url("css/style.css");
</style>
<meta name="google-translate-customization" content="f91f6b59f687e07-728df0675eb22298-gf4646ea128908eee-c"></meta>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getF['Name']; ?></td>
      	    <td><?php echo $row_getF['Movement']; ?></td>
      	    <td>
				<?php if ($row_getF['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getF['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getF['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getF['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getF['CityBirth']; ?></td>
      	    <td><?php echo $row_getF['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getF = mysql_fetch_assoc($getF)); ?>
    </tbody>
    </table>
  </section>
