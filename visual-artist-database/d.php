...
$query_getD = "SELECT * FROM artists WHERE artists.LastName LIKE 'D%' ORDER BY artists.LastName";
$getD = mysql_query($query_getD, $artists) or die(mysql_error());
$row_getD = mysql_fetch_assoc($getD);
...
<title>Visual Artist Database | D</title>
...
<meta name="description" content="Visual Artist Database" />
<meta name="author" content="Alex Taylor Swan" />
<title>AlexSwan.com | Visual Artist Database | D</title>
<style type="text/css" media="screen">
@import url("css/style.css");
</style>
<meta name="google-translate-customization" content="f91f6b59f687e07-728df0675eb22298-gf4646ea128908eee-c"></meta>
...
<header role="banner"><h1>Visual Artist Database</h1></header>
  ...
  <section role="main">
      ...
      <tbody>
      	<?php do { ?>
      	  <tr>
      	    <td><?php echo $row_getD['Name']; ?></td>
      	    <td><?php echo $row_getD['Movement']; ?></td>
      	    <td>
				<?php if ($row_getD['YearBirth']==0) {
					echo('Unknown'); }
						else {
							echo $row_getD['YearBirth']; } ?>
            </td>
      	    <td>
				<?php if ($row_getD['YearDeath']==0) {
					echo('Living'); }
						else {
							echo $row_getD['YearDeath']; } ?>
            </td>
      	    <td><?php echo $row_getD['CityBirth']; ?></td>
      	    <td><?php echo $row_getD['SimilarArtists']; ?></td>
   	      </tr>
      	  <?php } while ($row_getD = mysql_fetch_assoc($getD)); ?>
    </tbody>
    </table>
  </section>
